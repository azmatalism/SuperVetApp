                    ...
                    <View style={{ marginTop: 10, marginBottom: 6 }}>
                        {
                            chaacterImage ?
                                <TouchableOpacity activeOpacity={0.7}
                                    onPress={() => navigation.navigate(ROUTES.CHARACTER_PROFILE, { dataOne: item, dataTwo: item })}>
                                    <Image source={teamImage ? item.teamImage : item.characterImage} style={Styles.characterImages} />
                                </TouchableOpacity>
                                :
                                images ?
                                    <TouchableOpacity activeOpacity={0.7}
                                        onPress={() => navigation.navigate(ROUTES.BUNDLE_PURSHACE_PAGE, { dataOne: item, dataTwo: item })}>
                                        <Image source={images ? item.superImage : item.faeturedImage} style={Styles.margin5} />
                                    </TouchableOpacity>
                                    :
                                    <TouchableOpacity activeOpacity={0.7}
                                        onPress={() => navigation.navigate(ROUTES.PURSHACE_PAGE, { dataOne: item, dataTwo: item })}>
                                        <Image source={images ? item.superImage : item.faeturedImage} style={Styles.margin5} />
                                    </TouchableOpacity>
                        }
